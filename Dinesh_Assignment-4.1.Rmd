...
```{r}
0.000004985-0.000004255      
0.0002274-0.00022348       
```
...
```{r}
fit3=glm(Direction~Lag1+Lag2,data=weekly,family = binomial)
summary(fit3)
```
...
```{r}
fit4=glm(Direction~Lag1+Lag2,data = weekly[-1,],family = binomial)
```

##### (c) Use the model from (b) to predict the direction of the first observation.

```{r}
probs=predict(Q7_b.fit,weekly[1,],type = "response")>0.5
...
for (i in 1:dim(weekly)[1]){
  ...
  prob1=predict(fit_Q8,weekly[i,],type="response")>0.5
  Actual2=weekly[i,]$Direction=="Up"
  if(Actual2 != prob_Q8)
    error[i]=1
  }
sum(error_Q8)
